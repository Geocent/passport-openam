...
OpenAmStrategy.prototype.authenticate = function (req, options) {
  ...
  if (!token) {
    return this._handleInvalidToken(options.callbackUrl, req);
  }

  this._openam.isTokenValid(token, function (valid) {
    if (!valid) {
      return self._handleInvalidToken(options.callbackUrl, req);
    }
 ...
 * @api private
 */
OpenAmStrategy.prototype._handleInvalidToken = function (callbackUrl, req) {
  if (this._options.enableLoginRedirect) {
    callbackUrl = (callbackUrl || this._options.callbackUrl) || req.path;
