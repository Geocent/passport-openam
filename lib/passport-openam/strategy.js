...
var OpenAmStrategy = (function () {
    ...
    function _sanitizeOptions(options) {
        return {
          ...
          customLoginURL: options.customLoginURL || null
    };
    }
    ...
    var openAmStrategy = function (options, verify) {
        ...
        function _handleInvalidToken(req, redirect, fail) {
          ...
          if (_options.enableLoginRedirect) {
        	  if (_options.customLoginURL) {
        		  location = _options.customLoginURL + '?goto=' + _originalURL(req);
        	  } else {
            	  location = _openam.getLoginUiUrl({
        ...
        */
        function _loadRoles(token, attributes, callback) {
          var url = options.openAmBaseUrl + _openam._attributesUrl + '?subjectid=' + token + '&refresh=true';
          console.log('url:', url);
        ...
        /**
         * Request Policy decision. nodejs-openam only supports 'GET' so cannot use their call.
         *
         * @param {String} token
         * @param {String} uri
         * @param {String} method
         ...
         * @private
        */
        function _requestPolicy(token, uri, method, callback) {
          var url = options.openAmBaseUrl + _openam._authorizeUrl + '?subjectid=' + token + '&uri=' + uri + '&action=' + method;
          console.log('url:', url);
          ...
          _openam.isTokenValid(token, function (valid, error) {
            ...
            _loadUserAttributes(token, _loadRoles, function (err, attributes, roles) {
              ...
              if (_options.enforcePolicies) {
                _logger.debug('Encorcing policies: %s', _options.enforcePolicies);
                _requestPolicy(token, req.protocol + "://" + req.get('host') + req.path, req.method, function (err, decision) {

                  if (err) {
                    _logger.error('Unexpected error loading policies: %s', err);
                    return self.error(err);
                  }

                  _logger.debug('Policy decsion: %s', decision);
                  if (decision && decision.indexOf('true') >= 0) {
                    ...
                    _logger.error('Request denied');
                    return self.error('Request denied');
                  }
